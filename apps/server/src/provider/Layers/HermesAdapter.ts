      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const started = yield* Effect.gen(function* () {
            // Hermes requests structured user input via the standard ACP
            // `session/elicitation` method (not the proprietary
            // `x.ai/ask_user_question` extension that other providers use). Map the
            // form-mode schema properties to T3's `user-input.requested`
