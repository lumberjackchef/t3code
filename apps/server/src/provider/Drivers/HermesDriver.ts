/**
 * HermesDriver — `ProviderDriver` for the Hermes Agent runtime.
 *
 * Hermes exposes a standard ACP server (`hermes acp`) over stdio, so this
 * driver mirrors the Grok/Cursor ACP drivers: it builds an
 * `AcpSessionRuntime` on demand (spawning `hermes acp`), wires text
 * generation through the same runtime, and probes CLI version + model list
 * during the managed status check.
 *
 * Each `create()` captures the `HermesSettings` argument in closures owned
 * by the returned instance, so two instances (e.g. `hermes_personal` +
 * `hermes_work`) run fully independently with their own processes.
 *
...
// Hermes has no self-managed auto-update command in the provider sense; it
// upgrades through its own `hermes update`. Surface manual-only so T3 never
// tries to drive an unknown update command.
